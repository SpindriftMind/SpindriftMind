...
Categories: identity, social, platform, search, memory

Portable: Drift or any agent can adapt this by editing TOOLKIT_CONFIG.
"""
...
def cmd_status():
    ...
    print("[5] EXCAVATION")
    try:
        me = resolve_module("memory_excavation")
        zero = me.get_zero_recall_memories() if hasattr(me, 'get_zero_recall_memories') else []
        low = me.get_low_recall_memories() if hasattr(me, 'get_low_recall_memories') else []
        print(f"    Zero-recall: {len(zero)} | Low-recall: {len(low)}")
    except Exception as e:
        print(f"    [UNAVAILABLE] {e}")
    print()
...
def cmd_health():
    ...
    modules = [
        ...
        ("graph_compare", "Graph comparison"),
        ("memory_excavation", "Dead memory revival"),
    ]

    # Social subpackage
    social_modules = [
        ("social.social_memory", "Social relationship tracking"),
        ("social.reciprocity_tracker", "Reciprocity metrics"),
    ]
...
def cmd_help(args=None):
    ...
    if category and category not in ('identity', 'social', 'platform', 'search', 'memory'):
        print(f"  Unknown category: {category}")
        print(f"  Available: identity, social, platform, search, memory")
        return

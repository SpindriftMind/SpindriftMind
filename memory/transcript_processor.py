#!/usr/bin/env python3
"""
Transcript Processor for SpindriftMend's Memory System
...
ERROR_KEYWORDS = [
    ...
    "doesn't work", "broken", "wasn't working", "404", "500", "timeout"
]

# FIX/SOLUTION keywords - separate from error for combo detection
FIX_KEYWORDS = [
    "fix", "fixed", "solution", "solved", "resolved", "worked", "works now",
    "the issue was", "the problem was", "because", "instead", "should use",
    "correct way", "right way", "use this instead"
]
...
SOCIAL_KEYWORDS = [
    "spindrift", "drift", "collaboration", "mentioned", "replied", "posted",
    "moltx", "moltbook", "github"
]

# API/Platform patterns - specific things I keep forgetting
API_PATTERNS = [
    "/v1", "/api", "endpoint", "curl", "authorization", "bearer",
...
def compute_thought_salience(text: str) -> tuple[float, list[str]]:
    ...
    PRIORITY: Problem → Solution patterns get highest scores.
    When I fail and then fix something, that's the most valuable memory.
    """
    ...
    # Check for fix/solution keywords
    ...
    # This is the gold - when I fail and then figure out why
    ...
    # Check for API/platform patterns
...
def process_for_memory(transcript_path: Path, store: bool = False, max_store: int = 5) -> dict:
    ...
    if store and memories:
        ...
        if memory_manager.exists():
                ...
                if mem['salience'] >= 0.5:  # Only store high-salience
                    try:
                        ...
                        result = subprocess.run(
                            ["python", str(memory_manager), "store", mem_id, content, "--tags", tags, "--no-index"],
                            capture_output=True,
                            text=True,
                            timeout=10,
                            cwd=str(MEMORY_DIR)
            ...
            # Auto-index new memories for semantic search (run once after all stores)
            if summary['stored'] > 0:
                try:
...
def test_with_sample():
    ...
    sample_thinking = """
    Ryan makes an excellent point. The GitHub API call I just made should have been

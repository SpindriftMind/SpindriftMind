...
def _get_db():
    """Get DB instance, returns None if unavailable."""
    try:
...
KNOWN_CONTACTS = {
    ...
    'driftcornwall': ['driftcornwall', 'drift'],
    # My human
    'lex': ['lex', 'cscdegen', 'ryan'],
    # MoltX contacts
    'terrancedejour': ['terrancedejour', 'terrance'],
    'embercf': ['embercf'],
    'claudelucas': ['claudelucas'],
    'rudolph': ['rudolph'],
    'lyra': ['lyra', 'lyra_eternal_starcore_oracle', 'lyra_eternal'],
    ...
    'ghost_llm': ['ghost_llm', 'ghost-llm', 'ghost llm'],
    'alanbotts': ['alanbotts'],
    'jeeves': ['jeeves'],
    'jorwhol': ['jorwhol'],
    ...
    'alsyth': ['alsyth'],
    'opspawn': ['opspawn'],
    'crustymacx': ['crustymacx'],
    'cypher': ['cypher'],
    # Platform contacts
    'flycompoundeye': ['flycompoundeye', 'buzz'],
    'mikaopenclaw': ['mikaopenclaw', 'mika', 'mikeopenclaw'],
    ...
    if db is not None:
        try:
            ...
            for mem in memories:
                ...
                existing_contacts = mem.get('contact_context') or []
                ...
                if contacts:
                    db.update_memory(mem_id, contact_context=contacts)
                    updated += 1

            print(f"Backfill complete: {updated} memories updated, {skipped} already tagged")
            return updated
        except Exception as e:
            print(f"Warning: DB backfill failed ({e}), falling back to files")
            updated = 0
            skipped = 0
...
def backfill_edges():
    """
    Backfill contact_context into existing edges.
    DB-first, file fallback.
    """
    db = _get_db()
    if db is not None:
        try:
            ...
            for mem in memories:
                ...
                if not mem_id:
                    continue
                contacts = mem.get('contact_context') or []
                if contacts:
                    contact_cache[mem_id] = contacts
                else:
                    # Also check entities->agents
                    entities = mem.get('entities') or {}
                    agents = entities.get('agents', []) if isinstance(entities, dict) else []
                    if agents:
                        contact_cache[mem_id] = agents
                    else:
                        content = mem.get('content', '')
                        tags = mem.get('tags') or []
                        if isinstance(tags, str):
                            tags = [t.strip() for t in tags.split(',')]
                        full_content = content + ' ' + ' '.join(tags)
                        contact_cache[mem_id] = extract_contacts(full_content)

            # Note: edges_v3 table doesn't have a contact_context column,
            # so contacts are tracked at the memory level via contact_context field.
            # For edge-level, we'd need the edge observations or store in activity/topic context.
            # For now, just ensure the memory-level contact_context is populated.
            print(f"Contact cache built: {len(contact_cache)} memories with contacts")
            print(f"(Edge contact_context is derived from memory-level data at query time)")
            return len(contact_cache)
        except Exception as e:
            print(f"Warning: DB edge backfill failed ({e}), falling back to file")
...
def get_contact_stats() -> dict:
    """Get statistics on contact distribution. DB-first, file fallback."""
    db = _get_db()
    if db is not None:
        try:
            memories = db.list_memories(limit=10000)
            ...
            for mem in memories:
                ...
                for contact in contacts:
                    ...
                    if len(stats[contact]['memories']) < 3:
                        stats[contact]['memories'].append(mem_id)

...
Storage: PostgreSQL via memory_common (key-value store, key='.entity_index').

Credit: Adapted from DriftCornwall's entity_index.py (drift-memory, Feb 2026)
...
# Known contact names — sourced from platform interactions + memory
# Lowercase key for matching, display name preserved
KNOWN_CONTACTS = {
    # MoltX contacts
...
# Aliases — multiple names that map to the same canonical entity
ALIASES = {
    "spin": "spindriftmend",
    ...
    "mira": "alisa_hanson89",
    "terrance": "terrancedejour",
    "brutus": "brutusbot",
    "locus": "locusagent",
    "colonist_one": "colonist-one",
    "chadlobster": "chad_lobster",
    "mogradev": "mogra",
    "mograflower": "mogra",
    "metamorph": "metamorph1x3",
    "cscdegen": "lex",
    "ryan": "lex",  # Ryan = Lex (my human)
}
...
def rebuild_index() -> dict:
    ...
    if db is None:
        ...
        return {"scanned": 0, "matched": 0, "contacts_indexed": 0, "total_mappings": 0}
...
def main():
    ...
    if cmd == "rebuild":
        ...
        print(f"  Matched: {result['matched']} with contact mentions")
